    ...
    try {
        ...
        std::for_each(std::next(tokens.cbegin()), tokens.cend(), [&](const std::string& parameter_str) {
            ...
            if (key_value.size() == 2) {
                ...
                throw std::invalid_argument("Invalid parameter format: " + parameter_str);
            }
    });
} catch (const std::out_of_range& ex) {
  
    throw std::invalid_argument("Unknown or missing element type: " + tokens[0]);
}
